Feature: Claude CLI Client
  ...
  Scenario: Generate text from a prompt with default timeout
    Given a ClaudeCliClient instance with default timeout
    When generate is called with prompt "Write hello world"
    Then an object is returned with key "content" containing the response text
    And the object contains key "promptTokens" with value 0
    And the object contains key "completionTokens" with value 0
    And the object contains key "tokensUsed" with value 0
  ...
  Scenario: CLI subprocess times out
    ...
    Then an Error is thrown with a message indicating timeout
  ...
  Scenario: Response content is stripped of whitespace
    ...
    Then the returned object "content" value has whitespace removed
