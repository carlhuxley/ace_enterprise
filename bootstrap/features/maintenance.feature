Feature: Playbook Maintenance Operations
  ...
  Scenario: Decay confidence of stale bullets in a playbook
    Given a playbook manager with playbook "pb-001"
    And playbook "pb-001" has bullets with lastUsed older than 90 days
    And those bullets have confidenceScore of 0.5
    When decayStaleBullets is called with playbookId "pb-001", staleDays 90, and decayFactor 0.1
  ...
  Scenario: Decay does not affect recently used bullets
    Given a playbook manager with playbook "pb-002"
    And playbook "pb-002" has bullets with lastUsed within the last 30 days
    When decayStaleBullets is called with playbookId "pb-002", staleDays 90, and decayFactor 0.1
    Then the function returns 0

  Scenario: Decay confidence cannot go below zero
    Given a playbook manager with playbook "pb-003"
    And playbook "pb-003" has a stale bullet with confidenceScore 0.05
    When decayStaleBullets is called with playbookId "pb-003", staleDays 90, and decayFactor 0.1
    Then the function returns 1
    And the bullet has confidenceScore of 0.0

  Scenario: Prune low confidence bullets that are old enough
    Given a playbook manager with playbook "pb-004"
    And playbook "pb-004" has bullets with confidenceScore 0.05 and createdAt older than 30 days
    When pruneLowConfidenceBullets is called with playbookId "pb-004", minConfidence 0.1, and minAgeDays 30
  ...
  Scenario: Prune does not remove bullets that are too new
    Given a playbook manager with playbook "pb-005"
    And playbook "pb-005" has bullets with confidenceScore 0.05 and createdAt within the last 10 days
    When pruneLowConfidenceBullets is called with playbookId "pb-005", minConfidence 0.1, and minAgeDays 30
    Then the function returns 0

  Scenario: Prune does not remove bullets with sufficient confidence
    Given a playbook manager with playbook "pb-006"
    And playbook "pb-006" has bullets with confidenceScore 0.5 and createdAt older than 30 days
    When pruneLowConfidenceBullets is called with playbookId "pb-006", minConfidence 0.1, and minAgeDays 30
    Then the function returns 0

  Scenario: Run full maintenance cycle on a specific playbook
    Given a playbook manager with playbook "pb-007"
    And playbook "pb-007" has stale bullets and low confidence old bullets
    When runMaintenance is called with playbookId "pb-007", decayStaleDays 90, decayFactor 0.1, pruneThreshold 0.1, and pruneMinAgeDays 30
  ...
  Scenario: Run full maintenance cycle on all playbooks
    Given a playbook manager with playbooks "pb-008" and "pb-009"
