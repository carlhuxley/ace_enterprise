...
- [x] EnsembleLearner cross-model voting wired into `build_feature_ensemble` ([#6](https://github.com/carlhuxley/ace_enterprise/issues/6)) — opt-in `learn: true`: after winner selection, each candidate model independently reflects on the outcome, cross-votes on the resulting bullets, and consensus-approved bullets (higher initial confidence than single-model learning) land in the shared playbook
- [x] `build_feature_ensemble` for TypeScript and Go ([#7](https://github.com/carlhuxley/ace_enterprise/issues/7)) — `TypeScriptGenerationRubric` / `GoGenerationRubric` added alongside `CodeGenerationRubric`; the "tests" dimension runs the real sandboxed vitest / `go test` harness, same mechanism as Python
- [x] Opt-in SEARCH/REPLACE-block GREEN patching wired into a live entry point ([#55](https://github.com/carlhuxley/ace_enterprise/issues/55)) — `.ace/config.yaml` `diff_editing: true` / `ace tdd --diff-editing` turns on deterministic, host-side patch application instead of always regenerating the whole file once an implementation exists to patch against; default off, unbenchmarked
...
- [x] Reflector/Curator diagnosis persisted per attempt, harness-wide ([#46](https://github.com/carlhuxley/ace_enterprise/issues/46)) — `TDDCycleRunner._learn()` (every pod: Python/TypeScript/Go/Simulation) writes Reflector's diagnosis + Curator's delta bullets to a JSON file keyed by cycle, and `ace view` prints it alongside telemetry for archived `SimulationPod` attempts
